...
function AlbumImage({ item, albumName, index }: { item: { url: string; name: string }, albumName: string, index: number }) {
  ...
  return (
    <Link
      ...
      className="relative w-full block border border-transparent hover:border-sipes-blue dark:hover:border-sipes-orange"
    >
      <Image
...
function AlbumGridContent({ albumName }: { albumName: string }) {
